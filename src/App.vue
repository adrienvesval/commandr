...
<main>
  ...
  <section>
    <div class="table-list">
      ...
      <div class="cmd item" row v-for="command in commands" :key="command.id">
        ...
        <div class="day" row center tt="Click to See Logs" @click="popup_logs = command">
          <div class="cells" column v-for="hours_x2 in 12" :key="hours_x2">
            ...
            <div class="cell" :class="run.error ? 'negative' : 'positive'" v-for="run in command.runs.filter(d => d.start.slice(0, 10) === day && Math.trunc(d.start.slice(11, 13) / 2) === hours_x2)" :key="run.id" v-else></div>
  ...
  <drawer @close="popup_edit = null" :openned="!!popup_edit">
    <form class="cmd edit" column @submit.prevent="edit(popup_edit.id, $event.target)">
      ...
      <hr>
      <label>Task {{ popup_edit && popup_edit.id.replace('C', '#') }} will run every time Task #<input type="text" name="runhook" /> succeeds.</label>
      <hr>
      <label>Task {{ popup_edit && popup_edit.id.replace('C', '#') }} will run on schedule at: <input type="text" name="schedule" @focus="!$event.target.value && ($event.target.value = 'R/' + new Date().iso().slice(0, 13) + ':00/PT24H')" /></label>
      <hr>
      <label>If Task {{ popup_edit && popup_edit.id.replace('C', '#') }} succeeds, run: <input type="text" name="onsuccess" /></label>
      <hr>
      <label>If Task {{ popup_edit && popup_edit.id.replace('C', '#') }} fails, run: <input type="text" name="onerror" /></label>
      <button>SAVE</button>
    </form>
  </drawer>
  <drawer @close="popup_logs = null" :openned="!!popup_logs">
    <form class="cmd logs" column @submit.prevent="logs(popup_logs.id, $event.target)">
      ...
      <div v-for="(run, index) in popup_logs && popup_logs.runs" :key="index">
